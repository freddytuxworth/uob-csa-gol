...
func NewWindow(width, height int32) *Window {
	...
	util.Check(err)
	window, err := sdl.CreateWindow("GOL GUI", sdl.WINDOWPOS_CENTERED, sdl.WINDOWPOS_CENTERED, width * 2, height * 2, sdl.WINDOW_SHOWN)
	util.Check(err)
	renderer, err := sdl.CreateRenderer(window, -1, sdl.WINDOW_SHOWN)
	util.Check(err)
	sdl.SetHint(sdl.HINT_RENDER_SCALE_QUALITY, "nearest")
	err = renderer.SetLogicalSize(width * 2, height * 2)
	util.Check(err)
...
func (w *Window) RenderFrame() {
	...
	util.Check(err)
	err = w.renderer.Copy(w.texture, nil, &sdl.Rect{W: w.Width * 2, H: w.Height * 2})
	util.Check(err)
	w.renderer.Present()
}
